...
def data_clean(df1, df2):
    try:
        ...
        df_presc_sel = df_presc_sel.withColumn('years_of_exp', col('years_of_exp').cast(FloatType()))
        ...
        #created functions in validate.py and using it in driver.py file directly
        ...
        loggers.warning("fill the null values in tx_cnt with the avg value....")
        mean_tx_cnt=df_presc_sel.select(mean(col('tx_cnt'))).collect()[0][0]
        df_presc_sel=df_presc_sel.fillna(mean_tx_cnt,'tx_cnt')

        loggers.warning("successfully dropped the null values....")
